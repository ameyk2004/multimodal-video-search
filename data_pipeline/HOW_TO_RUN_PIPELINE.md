...
Processes queries generated by Gemini, embeds them, and uploads them to Qdrant.
```bash
python scripts/embed_queries.py
```
*   **What it does:** Extracts query lists from `data_pipeline/enriched_metadata/`, calls the Hugging Face API to generate embeddings, and uploads them to the `sadhananandadeep-queries` collection in Qdrant.
*   **Idempotent:** The script automatically queries Qdrant first and skips any queries that have already been uploaded.
